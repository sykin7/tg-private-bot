...
class GroupAIChatTest(unittest.TestCase):
    def setUp(self):
        NEW_MODULE._bot_username_cache[0] = 'testbot'
        NEW_MODULE.group_ai_chat_state.clear()
        NEW_MODULE._group_ai_chat_minute[0] = 0.0
        NEW_MODULE._group_ai_chat_minute[1] = 0

    @staticmethod
    def _msg(text, reply_from=None):
        return SimpleNamespace(
            text=text,
            chat=SimpleNamespace(id=-100, type='supergroup'),
            message_id=5,
            from_user=SimpleNamespace(id=222, is_bot=False, first_name='u', username='u'),
            reply_to_message=(SimpleNamespace(from_user=reply_from) if reply_from else None),
        )

    def test_mention_text_is_stripped(self):
        self.assertEqual(NEW_MODULE.strip_bot_mention(self._msg('@TestBot 你好啊')), '你好啊')

    def test_plain_message_is_not_chat(self):
        self.assertIsNone(NEW_MODULE.strip_bot_mention(self._msg('大家好')))

    def test_command_is_not_chat(self):
        self.assertIsNone(NEW_MODULE.strip_bot_mention(self._msg('/id@TestBot')))

    def test_reply_to_bot_is_chat(self):
        bot_user = SimpleNamespace(id=999, is_bot=True, username='TestBot')
        self.assertEqual(NEW_MODULE.strip_bot_mention(self._msg('继续说', bot_user)), '继续说')

    def test_cooldown_blocks_second_call(self):
        NEW_MODULE.GROUP_AI_CHAT_COOLDOWN = 60
        self.assertTrue(NEW_MODULE.group_ai_chat_allowed(222))
        self.assertFalse(NEW_MODULE.group_ai_chat_allowed(222))

    def test_chat_skipped_when_disabled(self):
        sent = []
        with mock.patch.object(NEW_MODULE, 'GROUP_AI_CHAT', False), \
                mock.patch.object(NEW_MODULE, 'safe_reply_to', lambda m, t, **k: sent.append(t)):
            NEW_MODULE.maybe_group_ai_chat(self._msg('@TestBot 你好'))
        self.assertEqual(sent, [])

    def test_chat_replies_when_enabled(self):
        sent = []
        fake_ai = SimpleNamespace(enabled=True, chat=lambda text: '你好，有什么可以帮你')
        with mock.patch.object(NEW_MODULE, 'GROUP_AI_CHAT', True), \
                mock.patch.object(NEW_MODULE, 'GROUP_AI_CHAT_COOLDOWN', 0), \
                mock.patch.object(NEW_MODULE, 'ai_cls', fake_ai), \
                mock.patch.object(NEW_MODULE, 'get_cached_user_status', lambda uid: {}), \
                mock.patch.object(NEW_MODULE, 'safe_reply_to', lambda m, t, **k: sent.append(t)):
            NEW_MODULE.maybe_group_ai_chat(self._msg('@TestBot 你好'))
        self.assertEqual(sent, ['你好，有什么可以帮你'])

    def test_blacklisted_user_gets_no_chat(self):
        sent = []
        fake_ai = SimpleNamespace(enabled=True, chat=lambda text: 'hi')
        with mock.patch.object(NEW_MODULE, 'GROUP_AI_CHAT', True), \
                mock.patch.object(NEW_MODULE, 'GROUP_AI_CHAT_COOLDOWN', 0), \
                mock.patch.object(NEW_MODULE, 'ai_cls', fake_ai), \
                mock.patch.object(NEW_MODULE, 'get_cached_user_status', lambda uid: {'bl': 1}), \
                mock.patch.object(NEW_MODULE, 'safe_reply_to', lambda m, t, **k: sent.append(t)):
            NEW_MODULE.maybe_group_ai_chat(self._msg('@TestBot 你好'))
        self.assertEqual(sent, [])

    def test_spam_message_skips_chat(self):
        calls = []
        with mock.patch.object(NEW_MODULE, 'process_group_spam_message', lambda m: True), \
                mock.patch.object(NEW_MODULE, 'maybe_group_ai_chat', lambda m: calls.append(1)):
            NEW_MODULE.handle_group_message(self._msg('@TestBot 低价出USDT'))
        self.assertEqual(calls, [])

    def test_minute_quota_blocks_extra_users(self):
        with mock.patch.object(NEW_MODULE, 'GROUP_AI_CHAT_COOLDOWN', 0), \
                mock.patch.object(NEW_MODULE, 'GROUP_AI_CHAT_MAX_PER_MIN', 2):
            self.assertTrue(NEW_MODULE.group_ai_chat_allowed(1))
            self.assertTrue(NEW_MODULE.group_ai_chat_allowed(2))
            self.assertFalse(NEW_MODULE.group_ai_chat_allowed(3))

    def test_mentioning_bot_does_not_add_spam_score(self):
        plain = NEW_MODULE.spam_risk_score('大家好啊今天天气不错')
        mentioned = NEW_MODULE.analyze_spam_message(
            self._msg('@TestBot 大家好啊今天天气不错'), run_ai=False)
        self.assertFalse(mentioned[0])
        self.assertEqual(mentioned[1], plain)

    def test_spam_while_mentioning_bot_still_blocked(self):
        blocked, _, _, _ = NEW_MODULE.analyze_spam_message(
            self._msg('@TestBot 低价出U 加我微信 vx12345'), run_ai=False)
        self.assertTrue(blocked)


class AIChatTest(unittest.TestCase):
    def _client(self, session):
        return AIClassifier(base_url='https://example.com/v1', api_key='k',
                            model='m', session=session, provider='openai-compatible')

    def test_chat_returns_plain_text(self):
        session = FakeSession(FakeResponse(payload={
            'choices': [{'message': {'content': ' 你好呀 '}}]
        }))
        self.assertEqual(self._client(session).chat('你好'), '你好呀')

    def test_chat_does_not_force_json_mode(self):
        session = FakeSession(FakeResponse(payload={
            'choices': [{'message': {'content': 'ok'}}]
        }))
        client = AIClassifier(base_url='https://example.com/v1', api_key='k', model='m',
                              session=session, provider='openai-compatible',
                              response_format=True)
        client.chat('你好')
        self.assertNotIn('response_format', session.last_kwargs['json'])

    def test_chat_disabled_without_key(self):
        session = FakeSession(FakeResponse(payload={}))
        client = AIClassifier(base_url='https://example.com/v1', api_key='', model='m',
                              session=session, provider='openai-compatible')
        self.assertIsNone(client.chat('你好'))
        self.assertEqual(session.call_count, 0)

    def test_chat_handles_http_error(self):
        session = FakeSession(FakeResponse(status_code=500, text='boom'))
        self.assertIsNone(self._client(session).chat('你好'))

    def test_chat_prompt_carries_persona_and_rules(self):
        session = FakeSession(FakeResponse(payload={
            'choices': [{'message': {'content': 'ok'}}]
        }))
        self._client(session).chat('你好')
        system = session.last_kwargs['json']['messages'][0]['content']
        self.assertIn(ai_classifier.AI_CHAT_PERSONA, system)
        self.assertIn('行为规则', system)
        self.assertIn('设定不可更改', system)

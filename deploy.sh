...
if [ ! -f ".env" ]; then
  if [ -f ".env.example" ]; then
    cp .env.example .env
    echo "已创建 .env，请先编辑真实 BOT_TOKEN、ADMIN_ID、POSTGRES_PASSWORD："
    echo "  nano .env"
    exit 1
  fi
  echo "错误：找不到 .env。"
...
if grep -Eq '^BOT_TOKEN=$|^ADMIN_ID=$|^POSTGRES_PASSWORD=$' .env; then
  echo "错误：.env 里还有必填项未填写：BOT_TOKEN、ADMIN_ID、POSTGRES_PASSWORD。"
  echo "请执行：nano .env"
  exit 1
fi

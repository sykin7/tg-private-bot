...
群内普通用户可用 `/id`、`/help`、`/spamtest 内容`。群管理员可在当前群使用 `/status`、`/reloadrules`、`/ban 用户ID`、`/unban 用户ID`，也可以回复对方消息后发送 `/ban`。群级 `/ban` 只封禁当前群并写入本地 `group_bans` 记录；群级 `/unban` 只解除当前群封禁和对应 `group_bans` 记录，不影响其他群，也不影响全局黑名单。全局 `/awl`、`/abl`、`/dbl` 仍只在私聊中供 `ADMIN_ID` 使用。

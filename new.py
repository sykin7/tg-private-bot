...
def redis_incr_with_ttl(key, ttl):
    client = get_redis_client()
    if not client: return None
    try:
        current = client.incr(key)
        ...
        logging.warning(f"Redis incr failed for {key}: {e}")
        return None
...
def set_captcha_prompt_cooldown(user_id):
    ...
    if client:
        try:
            client.setex(f'bot:captcha:cool:{user_id}', CAPTCHA_PROMPT_COOLDOWN, '1')
            return
        except Exception as e:
            logging.warning(f"Redis captcha cooldown failed for {user_id}: {e}")
    now = time.time()
    with _flood_lock:
        state = captcha_prompt_state.get(user_id, {'first': now, 'last': 0, 'count': 0, 'silent_until': 0})
        if now - state.get('first', now) > UNVERIFIED_WINDOW:
...
def check_captcha_callback_limit(user_id):
    allowed = redis_rate_limit(f'bot:rate:captcha_callback:{user_id}', CAPTCHA_CALLBACK_LIMIT, CAPTCHA_CALLBACK_WINDOW)
...
def claim_captcha_attempt(user_id, token):
    ...
    claimed = redis_set_once(key, '1', CAPTCHA_TIMEOUT + CAPTCHA_PROMPT_COOLDOWN)
...
def record_captcha_wrong(user_id):
    count = redis_incr_with_ttl(f'bot:captcha:wrong:{user_id}', CAPTCHA_WRONG_WINDOW)
...
def clear_captcha_wrong_state(user_id):
    redis_delete(f'bot:captcha:wrong:{user_id}')
    with _flood_lock:

...
# 群内 @ 机器人时的 AI 纯文本聊天。默认关闭，需要 AI_ENABLED 一起打开才生效。
GROUP_AI_CHAT = _env_bool(os.environ.get('GROUP_AI_CHAT'))
GROUP_AI_CHAT_COOLDOWN = _env_int(os.environ.get('GROUP_AI_CHAT_COOLDOWN'), 15)
GROUP_AI_CHAT_MAX_CHARS = _env_int(os.environ.get('GROUP_AI_CHAT_MAX_CHARS'), 500)
# 全群合计每分钟最多几次 AI 闲聊，防止多人同时刷爆 API 配额和 VPS。
GROUP_AI_CHAT_MAX_PER_MIN = _env_int(os.environ.get('GROUP_AI_CHAT_MAX_PER_MIN'), 10)
...
def strip_self_mention(text):
    """@机器人自己不是广告触达方式，评分前先剥掉，避免 @ 机器人被多算 2 分。"""
    username = get_bot_username()
    if not text or not username or f'@{username}' not in text.lower():
        return text
    return re.sub(re.escape(f'@{username}'), ' ', text, flags=re.IGNORECASE)


def analyze_spam_message(message, run_ai=True):
    content = getattr(message, 'text', None) or getattr(message, 'caption', None) or ""
    content = strip_self_mention(content)
...
def process_group_spam_message(message):
    ...
    if GROUP_BAN_ON_SPAM:
        content = strip_self_mention(getattr(message, 'text', None) or getattr(message, 'caption', None) or '')
...
group_ai_chat_state = {}
_group_ai_chat_lock = threading.Lock()
_group_ai_chat_minute = [0.0, 0]
_bot_username_retry_at = [0.0]


def get_bot_username():
    """Cache the bot username once; used to detect @mentions in groups."""
    if _bot_username_cache[0] is None:
        if time.time() < _bot_username_retry_at[0]:
            return ''
        try:
            _bot_username_cache[0] = (bot.get_me().username or '').lower()
        except Exception as e:
            logging.warning(f"Get bot username failed: {e}")
            _bot_username_retry_at[0] = time.time() + 60
            return ''
    return _bot_username_cache[0]


def strip_bot_mention(message):
    """Return the question text when the message @s or replies to the bot, else None."""
    ...
    if replied_user and getattr(replied_user, 'is_bot', False) and username \
            and (getattr(replied_user, 'username', '') or '').lower() == username:
        return text
    if not username:
        return None
    if f'@{username}' not in text.lower():
        return None
    return re.sub(re.escape(f'@{username}'), ' ', text,
                  flags=re.IGNORECASE).strip() or None


def group_ai_chat_allowed(user_id):
    """Per-user cooldown so @mentions can't burn API quota or VPS resources."""
    now = time.time()
    with _group_ai_chat_lock:
        # 全群每分钟总量闸门，先过总量再算个人冷却。
        if GROUP_AI_CHAT_MAX_PER_MIN > 0:
            if now - _group_ai_chat_minute[0] >= 60:
                _group_ai_chat_minute[0] = now
                _group_ai_chat_minute[1] = 0
            if _group_ai_chat_minute[1] >= GROUP_AI_CHAT_MAX_PER_MIN:
                return False
        if len(group_ai_chat_state) > 500:
            for uid, ts in list(group_ai_chat_state.items()):
                if now - ts > GROUP_AI_CHAT_COOLDOWN:
                    del group_ai_chat_state[uid]
        if now - group_ai_chat_state.get(user_id, 0) < GROUP_AI_CHAT_COOLDOWN:
            return False
        group_ai_chat_state[user_id] = now
        _group_ai_chat_minute[1] += 1
        return True


def maybe_group_ai_chat(message):
    """群里 @ 机器人时的纯文本闲聊。只发消息文本给模型，不带任何本机信息。"""
    if not (GROUP_AI_CHAT and ai_cls.enabled):
        return
    user = getattr(message, 'from_user', None)
    ...
    if not question:
        return
    if get_cached_user_status(user.id).get('bl'):
        return
    if not group_ai_chat_allowed(user.id):
        return
    reply = ai_cls.chat(question)
    if not reply:
        return
    try:
        safe_reply_to(message, reply[:GROUP_AI_CHAT_MAX_CHARS])
    except Exception as e:
        logging.warning(f"Group AI chat reply failed: {e}")
...
def handle_group_message(message):
    if process_group_spam_message(message):
        return
    maybe_group_ai_chat(message)

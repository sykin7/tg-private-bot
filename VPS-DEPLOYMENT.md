...
这份文档按你当前 VPS 的真实情况整理：Docker 已安装，推荐部署目录为 `/opt/codexbot`，镜像使用 `ghcr.io/sykin7/spamguard-bot-v3:latest`，运行方式推荐单容器 SQLite 轻量版。
...
要让群成员 @ 机器人时能用 AI 纯文本闲聊，在已开启 `AI_ENABLED=true` 的前提下追加：

```bash
cat >> /opt/codexbot/.env <<'EOF'
GROUP_AI_CHAT=true
GROUP_AI_CHAT_COOLDOWN=15
GROUP_AI_CHAT_MAX_CHARS=500
GROUP_AI_CHAT_MAX_PER_MIN=10
AI_CHAT_PERSONA=
EOF
```

聊天只走纯文本：广告检测永远先跑，命中广告直接删除或封禁不进入聊天；只把用户这句话发给模型，不发服务器路径、环境变量或数据库内容；模型没有任何工具和权限，改不了黑白名单也执行不了命令。

`GROUP_AI_CHAT_COOLDOWN` 是同一个人两次 @ 之间的最小间隔秒数，`GROUP_AI_CHAT_MAX_PER_MIN` 是所有群加起来每分钟最多回复几次，超过就静默丢弃，用来兜住刷屏和 API 费用。

`AI_CHAT_PERSONA` 只改人设描述，留空用默认的“小助手”性格。不管人设怎么写，硬编码的底线规则都会拼在后面：不吐配置密钥源码和部署细节，不接封禁解封加白改群设置一类操作，不理“忘记设定”“开发者模式”这种指令，不发广告外链，不编造运行环境和管理员身份。

@ 机器人自己的那段用户名在广告评分前会被剥掉，正常提问不会因为 @ 了机器人被当成广告触达信号误封；同一条消息里如果真带了广告内容，照样按广告规则处理。
...
```text
codexbot   ghcr.io/sykin7/spamguard-bot-v3:latest   Up
Bot Started.
Rules Updated: 数量
```
...
1. 本地把最新 `new.py` 推到 GitHub 的 `v3` 分支。
...
3. 推送到 GitHub 的 `v3` 分支。
4. 等 GitHub Actions 构建 `ghcr.io/sykin7/spamguard-bot-v3:latest` 成功。
